...
#        self.root.deiconify()
    ...
    def updateGrid(self):
        self._currentGame = self.blankGrid(self._currentGameGrid)                     # list -- ints + ''
        self.clearGrid()
        self._grid.insert(self._currentGame)
        self._insertGrid()
        #self.checkGrid()
    ...
    def clearGrid(self):
        zeroGrid = ["" for i in range(81)]
        self._grid.insert(zeroGrid)
        self._insertGrid()
    ...
    def play_the_game(self):
        ...
        if self._currentGameGrid.min() > 0:
            grid.insert(self._currentGameGrid.flatten())
            if grid.gridCheck():
  ...
  #      return  (i, j), val, grid
    ...
    def alphabetize(self):
        self._grid.recodeAlphaNum(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'])
        self._insertGrid()
        self._isABC = True
    ...
    def recode(self) -> None:
        ...
        self._grid.recodeAlphaNum(nextGrid) 
        self._isABC = False
        self._insertGrid()
        self.getPermutations()
